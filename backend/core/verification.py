...
class AliyunVerification:
    ...
    def generate_c_value(self):
        """生成c参数值(时间戳:随机数)"""
        return f"{int(time.time()*1000)}:{random.random()}"
    
    def generate_umid_token(self):
        """生成umidToken"""
        timestamp = int(time.time()*1000)
        return f"defaultToken1_awsc_not_loaded@@{self.survey_url}@@{timestamp}"
    ...
    def get_verification(self, b_value=""):
        ...
        # 生成umidToken
        umid_token = self.generate_umid_token()

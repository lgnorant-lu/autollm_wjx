...
class OpenAIProvider(BaseLLMProvider):
    ...
    def generate(self, prompt: str, **kwargs) -> str:
        ...
        try:
            # 如果启用了模拟模式，返回模拟响应
            if self.mock:
                logger.info(f"[模拟模式] OpenAI生成文本, 提示词: {prompt[:50]}...")
                return f"[模拟响应] 这是来自OpenAI的模拟响应。提示词: {prompt[:30]}..."
    ...
    def generate_json(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            # 如果启用了模拟模式，返回模拟响应
            if self.mock:
                logger.info(f"[模拟模式] OpenAI生成JSON, 提示词: {prompt[:50]}...")
                return {
                    "mock": True,
                    "provider": "OpenAI",
                    "prompt": prompt[:50] + "...",
            ...
            logger.error(f"OpenAI生成JSON失败: {str(e)}")
            raise

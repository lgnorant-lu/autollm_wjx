...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/proxy_usage.log', encoding='utf-8'),
...
class IPUsageTracker:
    def __init__(self, log_file='data/ip_usage.json'):
...
def get_current_ip(proxy_url: str) -> str:
    """获取当前使用的IP"""
    try:
...
class SurveyTask:
    ...
    def __init__(self, survey_url, questions_path, target_num=100, max_workers=5, use_proxy=False, proxy_url=None):
        """
        初始化问卷任务
...
def cleanup_data_files(directory="data", keep_days=7, pattern=None):
    """
    清理旧的数据文件
...
def main():
    ...
    # 设置日志文件名
    log_file = f'logs/survey_{survey_id}_{timestamp}.log'
    ...
    # 清理旧的数据文件
    cleanup_data_files(directory="data", keep_days=30, pattern="*.json")
    ...
    # 保存统计信息到自动命名的JSON文件
    stats_file_path = f"data/stats_{survey_id}_{timestamp}.json"
    with open(stats_file_path, 'w', encoding='utf-8') as f:
        json.dump(stats, f, ensure_ascii=False, indent=4)
    logger.info(f"统计信息已保存到 {stats_file_path}")
    
    # 创建自动命名的IP使用记录文件
    ip_usage_file = f"data/ip_usage_{survey_id}_{timestamp}.json"
    ...
    # 优化问卷数据结构
    optimized_file = f"data/questions_optimized_{survey_id}_{timestamp}.json"
    ...
    if use_llm:
        try:
            ...
            # 保存生成的答案到文件
            with open("data/llm_answers.json", "w", encoding="utf-8") as f:
                json.dump(llm_answers, f, ensure_ascii=False, indent=2)
            
            logger.info("LLM生成的答案已保存")
    ...
    task = SurveyTask(
        ...
        # use_proxy=True,
        use_proxy=False,
        proxy_url="https://service.ipzan.com/core-extract?num=1&no=20250215419627045302&minute=1&format=txt&protocol=1&pool=ordinary&mode=whitelist&secret=gqtfq64vobjvs8"
    )
    ...
    # 清理旧的数据文件
    cleanup_data_files(pattern="*.json", keep_days=30)  # 保留30天内的JSON文件
